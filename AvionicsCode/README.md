# ALTIS Rocket Avionics System
**ESP32 기반의 2단 로켓 비행 컴퓨터(Flight Computer) 펌웨어**입니다.
IMU와 기압 센서 데이터를 센서 퓨전(Sensor Fusion)하여 비행 상태를 실시간으로 판단하며, 설정된 로직에 따라 서보모터 또는 파이로(Pyro) 장치를 제어하여 사출 및 단분리 미션을 수행합니다.
...
| **IMU (BMI088)** | 34, 35 | INT1,INT3 (Accel, Gyro) |
| **Barometer (BMP388)** | 32 | CS |
| **Barometer (BMP388)** | 39 | INT |
| **Servo 1~3** | 26, 27, 22 | Eject, Separation Control |
...
## Bluetooth Commands
비행 전 블루투스 터미널을 통해 시스템을 제어할 수 있습니다.
...
## Changelog
- **2025-12-29**: 
  - `Recovery` 시스템 라이브러리화 완료 (서보/파이로 통합 제어 구조)
  - 배터리 전압 모니터링 기능 추가 (블루투스 상태 전송)
  - 비행 로직(FlightLogic) 및 상태 머신 안정화
