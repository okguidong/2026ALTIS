...
void flightTask(void *pvParam)
{
    ...
    while (true)
    {
        ...
        if(logic.isSeparation()){
            SerialBT.printf("time: %d , sep\n",millis()-logic._launchTime);
            Serial.printf("time: %d , sep\n",millis()-logic._launchTime);
            recovery.Separate();
        }  
        if(logic.isEJect1()){
            SerialBT.printf("time: %d , EJ1\n",millis()-logic._launchTime);
            Serial.printf("time: %d , EJ1\n",millis()-logic._launchTime);
            recovery.EJ1();
        }
        if(logic.isEJect2()){
            SerialBT.printf("time: %d , EJ2\n",millis()-logic._launchTime);
            Serial.printf("time: %d , EJ2\n",millis()-logic._launchTime);
            recovery.EJ2();
            if(data.alt_baro < CHANGED_CHUTE_ALTITUDE){
                SerialBT.printf("time: %d , suit\n",millis()-logic._launchTime);
                Serial.printf("time: %d , suit\n",millis()-logic._launchTime);
                recovery.trigger(CHANGED_CHUTE);
            }
        }
        if (sensor_update != 0)
        {
...
void setup()
{
    ...
    while (!isArmed)
    {
        ...
        if (millis() - time > 1000)
        {
            ...
            float st = SEPARATION_TIMEOUT_MS/1000;
            float ej1t =EJECT1_TIMEOUT_MS/1000;
            float ej2t = EJECT2_TIMEOUT_MS/1000;
            SerialBT.printf("Vattery Voltage: %.2f V , Sea Level hPa: %f\n Seperate Time : %f , EJ1 Time : %f , EJ2 Time : %f \n", 
                sum * 0.0006667, navigation._seaLevelPa/100, st, ej1t, ej2t);
    ...
    recovery.begin();
    SerialBT.println("Launching System...");
